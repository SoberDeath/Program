...
class ProfileEditor(ctk.CTkFrame):
    def __init__(self, master, profile, save, cancel, delete, duplicate, shortcut, process_manager, executor):
        ...
        PageHeader(self, "Profile Editor", "Configure the apps and files that shape this workspace.", "Save Profile", self._save).pack(fill="x", pady=(0, 16))
        ...
        opens = self._section("Items to Open", "Add applications, documents, shortcuts, scripts, or folders. Windows opens files with their default application.")
        buttons = ctk.CTkFrame(opens, fg_color="transparent"); buttons.pack(fill="x", padx=18, pady=(2, 10))
        ctk.CTkButton(buttons, text="+  Add File / Application", command=self._add_file, fg_color=COLORS["accent"], width=175).pack(side="left")
        ctk.CTkButton(buttons, text="+  Add Folder", command=self._add_folder, fg_color=COLORS["surface_hover"], width=125).pack(side="left", padx=(8, 0))
    ...
    def _append_open_item(self, path: str):
        if path and not any(a.get("path", "").casefold() == path.casefold() for a in self.item["open_apps"]):
            target = Path(path)
            self.item["open_apps"].append({"name": target.stem if target.is_file() else target.name, "path": path})
            self._render_lists()

    def _add_file(self):
        path = filedialog.askopenfilename(
            title="Select an application or file",
            filetypes=[
                ("All supported items", "*.*"),
                ("Applications", "*.exe"),
                ("Shortcuts", "*.lnk"),
                ("Documents", "*.pdf;*.doc;*.docx;*.xls;*.xlsx;*.ppt;*.pptx;*.txt;*.csv"),
                ("Scripts", "*.bat;*.cmd;*.ps1;*.py"),
                ("All files", "*.*"),
            ],
        )
        self._append_open_item(path)

    def _add_folder(self):
        self._append_open_item(filedialog.askdirectory(title="Select a folder"))
    ...
    def _render_lists(self):
        ...
        for index, app in enumerate(self.item["open_apps"]): self._list_row(self.open_list, app.get("name", "Item"), app.get("path", ""), lambda i=index: (self.item["open_apps"].pop(i), self._render_lists()), app.get("path", ""))
