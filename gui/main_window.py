...
from app_version import APP_VERSION, APP_VERSION_SHORT
...
class MainWindow(ctk.CTk):
    ...
    def __init__(self, config, profiles, processes) -> None:
        ...
        self.title(f"AppManager {APP_VERSION_SHORT}")
    ...
    def _apply_app_icon(self) -> None:
        try:
            icon = self._resource_path("assets/appmanager.ico")
            if icon.exists():
    ...
    def _build_sidebar(self) -> None:
        ...
        ctk.CTkLabel(info, text=f"AppManager  v{APP_VERSION_SHORT}", text_color=COLORS["text"], font=(FONT, 10, "bold"), anchor="w").pack(fill="x")
        ctk.CTkLabel(info, text="Offline workspace launcher", text_color=COLORS["muted"], font=(FONT, 9), anchor="w").pack(fill="x", pady=(3, 0))
        ctk.CTkLabel(info, text="LOCAL  •  PRIVATE", text_color=COLORS["muted"], font=(FONT, 9, "bold"), anchor="w").pack(fill="x", pady=(8, 0))
    ...
    def edit_profile(self, profile: dict[str, Any] | None) -> None:
        ...
        self.editor = ProfileEditor(self.content, profile, self._save_profile, lambda: self.show("Dashboard"), self._delete_profile, self._duplicate_profile, self._shortcut, self.processes, self.executor)
    ...
    def _shortcut(self, item: dict[str, Any]) -> None:
        ...
        self.executor.submit_ui(ShortcutManager().create, lambda result: self.show_status("success" if result[0] else "warning", result[1]), item["name"])
    ...
    def _start_tray(self) -> None:
        ...
        def create_tray():
            ...
            tray = TrayManager(lambda: self.executor.post_ui(self.restore), lambda profile: self.executor.post_ui(lambda: self.run_profile(profile)), lambda: self.executor.post_ui(self.exit_app))
    ...
    def show_status(self, status: str, message: str) -> None:
        self.activity.grid()
        self.activity.reset("AppManager")
